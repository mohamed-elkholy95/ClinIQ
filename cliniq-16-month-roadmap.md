...
- [x] **Python SDK v0.3.0 — evaluation framework and conversation memory endpoints** — Expanded SDK from 29 to 31 endpoint groups, adding typed client methods, response dataclasses, and comprehensive test coverage for the two newest API modules:
  ...
  - **Conversation memory (5 new client methods)**: `add_conversation_turn()` (record analysis turns with entities, ICD codes, risk scores, summary, document_id, arbitrary metadata), `get_conversation_context()` (retrieve aggregated session context with configurable last_n), `clear_conversation()` (DELETE session history), `get_conversation_stats()` (memory usage statistics), `list_conversation_sessions()` (active sessions with turn counts and timestamps)
  - **10 new response dataclasses**: `ClassificationEvalResult` (MCC + confusion matrix + calibration dict), `KappaResult` (kappa + observed/expected agreement), `NEREvalResult` (exact/partial/type-weighted F1 + match counts), `ROUGEEvalResult` with nested `ROUGEScores` (P/R/F1 per variant + length metrics), `ICDEvalResult` (3-level accuracy + match counts), `AUPRCResult` (label + score + class statistics), `ConversationTurnResult` (session_id + turn_id + count), `ConversationContext` (unique entities/ICD/risk trend + context list), `ConversationStats` (active sessions + total turns + config), `ConversationSessionInfo` (session metadata with timestamps)
  - **HTTP helper**: Added `_delete()` method to ClinIQClient for DELETE requests (used by `clear_conversation()`)
  - **SDK version**: 0.2.0 → 0.3.0; `__init__.py` updated with all 15 new public exports
  - **SDK README**: Updated method tables with evaluation framework (7 endpoints) and conversation memory (5 endpoints) sections; models documentation expanded
- [x] **49 new SDK tests** (was 78, now 127): `test_client.py` (31 new — EvaluateClassification 3, EvaluateAgreement 1, EvaluateNER 2, EvaluateROUGE 1, EvaluateICD 1, EvaluateAUPRC 2, ListEvaluationMetrics 1, AddConversationTurn 2, GetConversationContext 1, ClearConversation 1, ConversationStats 1, ListConversationSessions 2), `test_models.py` (18 new — ClassificationEvalResult 3, KappaResult 2, NEREvalResult 2, ROUGEEvalResult 3, ICDEvalResult 2, AUPRCResult 2, ConversationTurnResult 1, ConversationContext 2, ConversationStats 2, ConversationSessionInfo 1)
- [x] **Total test suite: 3062 passing** (backend: 3062, frontend: 596, SDK: 127), 0 failures
